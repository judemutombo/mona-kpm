...
if(DbAuth::getAuth(App::getInstance()->get_Db())->isConnect()){
    if(isset($_POST["id"]) && isset($_POST["offer"]) && isset($_POST["devise"]))
    {
        ...
        if(count($product) == 1)
        {
            ...
            if($db->inserer("INSERT INTO offer(product,sender,receiver,state_,date,price_,code,devise_) 
            VALUES(?,?,?,?,?,?,?,?)",[$_POST["id"],$_SESSION["user"],$product[0]->owner,"waiting",date("Y-m-d"),$_POST["offer"],$code,$_POST["devise"]])){
                $content = $user->name." ".$user->surname." sent you an offer";
                if($db->inserer("INSERT INTO notification(content,type,user,ref_offer,prod_ref) VALUES(?,?,?,?,?) ",[$content,"offer",$product[0]->owner,$code,$product[0]->prod_code]))
                {
