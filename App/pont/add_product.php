...
if(DbAuth::getAuth(App::getInstance()->get_Db())->isConnect()){
    if(!isset($_SESSION["prod_image"]) || count($_SESSION["prod_image"]) < 4 )
    {
        ...
        if(isset($_POST["name"]) && isset($_POST["price"]) && isset($_POST["devise"]) && isset($_POST["category"]) && isset($_POST["description"]) && isset($_POST["condition"]))
        {
            ...
            $query = "INSERT INTO product (`describe`, `condition`, price,state, devise, owner, name_, prod_code, category".$part.")VALUES (?,?,?,?,?,?,?,?,?".$part2.")";
            $parametres = [$_POST["description"],$_POST["condition"],$_POST["price"],"sale",$_POST["devise"],$_SESSION['user'],$_POST["name"],$code,$_POST["category"]];
